...
{
    static RenderTexture canvasTexture = LoadRenderTexture(GetScreenWidth(), GetScreenHeight());
    ...
    bool screenSizeChanged = (canvasSize.height != boundingBox.height || canvasSize.width != boundingBox.width);

    if (screenSizeChanged) {
        canvasTexture = LoadRenderTexture(boundingBox.width, boundingBox.height);
    }

    Rectangle canvasRec = { .x = 0, .y = 0, .width = boundingBox.width, .height = -boundingBox.height };
    ...
    EndMode3D();
    EndTextureMode();
    DrawTextureRec(canvasTexture.texture, canvasRec, (Vector2) { boundingBox.x, -boundingBox.y }, WHITE);
